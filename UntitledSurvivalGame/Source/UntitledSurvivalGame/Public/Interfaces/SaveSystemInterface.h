// Fill out your copyright notice in the Description page of Project Settings.

#pragma once

#include "CoreMinimal.h"
#include "UObject/Interface.h"
#include "Game/SaveGame_SurvivalGame.h"
#include "SaveSystemInterface.generated.h"

class USaveGame_SurvivalGame;

// This class does not need to be modified.
UINTERFACE(MinimalAPI)
class USaveSystemInterface : public UInterface
{
	GENERATED_BODY()
};

/**
 * 
 */
class UNTITLEDSURVIVALGAME_API ISaveSystemInterface
{
	GENERATED_BODY()

	// Add interface functions to this class. This is the class that will be inherited to implement this interface.
public:
	UFUNCTION(BlueprintCallable, BlueprintNativeEvent)
	void SaveActorData(USaveGame_SurvivalGame* SaveGameRef);

	UFUNCTION(BlueprintCallable, BlueprintNativeEvent)
	void LoadActorData(USaveGame_SurvivalGame* SaveGameRef);

	UFUNCTION(BlueprintCallable, BlueprintNativeEvent)
	void OnLoadedFromSave();

};
